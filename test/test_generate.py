...
def test_make_braille__is_valid() -> None:
    braille=_make_braille()

    assert len(braille) == 3
    assert len(braille[0]) == len(braille[1]) == len(braille[2]) == 2

def test__is_valid_with_invalid_braille_returns_false() -> None:
...
def test__is_valid_with_valid_braille_returns_true() -> None:
    assert _is_valid((
        (0, 0),
        (0, 0),
        (0, 0)
...
def test_make_valid_braille_returns_valid_braille() -> None:
    assert all([
